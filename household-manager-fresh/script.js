...
class HouseholdManager {
    constructor() {
        ...
        this.currentUserId = '1'; // Default to first user
    ...
    setupEventListeners() {
        ...
        document.getElementById('bills-search-input').addEventListener('input', (e) => this.searchBills(e.target.value));
    ...
    setupModalListeners() {
        ...
        // Add Chore Modal (Management)
        document.getElementById('add-chore-manage-btn').addEventListener('click', () => this.openModal('add-chore-modal'));
    ...
    saveRoommate() {
        ...
        const color = document.getElementById('roommate-color').value;
    ...
    renderRoommates() {
        ...
        this.roommates.forEach(roommate => {
            ...
            card.style.borderLeftColor = roommate.color;
            
            card.innerHTML = `
                <div class="roommate-name" style="color: ${roommate.color}">${roommate.name}</div>
                ${roommate.email ? `<div class="roommate-email">${roommate.email}</div>` : ''}
                <div class="roommate-actions">
                    <button class="btn btn-danger btn-small" onclick="app.deleteRoommate('${roommate.id}')">
                    ...
                    <div class="chore-detail">
                        ...
                        <div class="chore-detail-value">
                            <div class="chore-assignee">
                                <div class="chore-assignee-color" style="background-color: ${roommate ? roommate.color : '#1a73e8'}"></div>
                                ${roommate ? roommate.name : 'Unknown'}
                            </div>
                        </div>
                    </div>
                    <div class="chore-detail">
    ...
    renderPersonalChores() {
        ...
        if (myChores.length === 0) {
            container.innerHTML = '<p class="text-center">No chores assigned to you yet.</p>';
            return;
        }
    ...
    rescheduleLaundryBooking(bookingId) {
        ...
        document.getElementById('laundry-date').value = this.formatDate(startTime, { year: 'numeric', month: '2-digit', day: '2-digit' });
    ...
    createChoreElement(chore) {
        ...
        element.style.backgroundColor = roommate ? roommate.color : '#1a73e8';
        element.textContent = chore.name;
        const assignedToDisplay = roommate ? roommate.name + (roommate.id === this.settings.currentUserId ? ' (You)' : '') : 'Unknown';
    ...
    getChoresForDate(date) {
        return this.chores.filter(chore => {
            switch (chore.frequency) {
                case 'none':
                    return this.isSameDay(date, new Date(chore.createdAt));
                case 'daily':
                    return true;
                case 'weekly':
                    return this.isSameDayOfWeek(date, new Date(chore.createdAt));
                case 'monthly':
                    return date.getDate() === new Date(chore.createdAt).getDate();
                default:
                    return false;
            }
        });
    ...
    renderLaundrySchedule() {
        ...
        for (let hour = 8; hour < 22; hour++) {
            ...
            weekDays.forEach(day => {
                ...
                cell.dataset.date = this.formatDate(day, { year: 'numeric', month: '2-digit', day: '2-digit' });
                ...
                if (isBooked) {
                    ...
                    const userNameDisplay = booking.userName + (booking.userId === this.settings.currentUserId ? ' (You)' : '');
                    cell.title = `${userNameDisplay} - ${bookingType.toUpperCase()} - ${dayName} ${startTimeStr} to ${endTimeStr}${booking.notes ? ` (${booking.notes})` : ''}`;
                } else {
    ...
    handleTimeCellClick(e, cell) {
        ...
        // If clicking on an already selected cell, do nothing
    ...
    updateSelectionInfo() {
        ...
        selectionText.textContent = `${dayName} ${startTime} - ${endTime}`;
    ...
    openLaundryBookingModal() {
        ...
        document.getElementById('laundry-date').value = date;
    ...
    saveLaundryBooking() {
        ...
        // Use current user ID
        const userId = this.settings.currentUserId;
        const startDateTime = new Date(`${date}T${startTime}`);
        const endDateTime = new Date(`${date}T${endTime}`);
        const roommate = this.roommates.find(r => r.id === userId);
        ...
        if (bookingId) {
            ...
            if (booking) {
                ...
                booking.userName = roommate.name;
                booking.type = type;
                booking.notes = notes;
            ...
            this.addNotification(
                'Laundry Booking',
                `${roommate.name} booked ${type} from ${this.formatTime(startDateTime)} to ${this.formatTime(endDateTime)}`,
                'fa-tshirt',
                'booking'
            );
        }
    ...
    saveMaintenanceIssue() {
        ...
        const reporterName = this.roommates.find(r => r.id === reporter)?.name || 'Unknown';
            ...
            item.innerHTML = `
                ...
                <div class="maintenance-item-details">
                    ...
                    <div class="maintenance-detail">
                        ...
                        <div class="maintenance-detail-value">${issue.reporterName}</div>
                    </div>
                    <div class="maintenance-detail">
    ...
    initializeUSCRoommates() {
        const uscRoommates = [
            { id: 'user1', name: 'Alex Chen', email: 'alex.chen@usc.edu', avatar: '👨‍💻', preferences: { chores: ['dishes', 'trash'], availability: 'weekends' } },
            { id: 'user2', name: 'Sarah Johnson', email: 'sarah.j@usc.edu', avatar: '👩‍🎓', preferences: { chores: ['cleaning', 'laundry'], availability: 'weekdays' } },
            { id: 'user3', name: 'Mike Rodriguez', email: 'mike.r@usc.edu', avatar: '👨‍🍳', preferences: { chores: ['cooking', 'groceries'], availability: 'evenings' } },
            { id: 'user4', name: 'Emma Davis', email: 'emma.d@usc.edu', avatar: '👩‍🔬', preferences: { chores: ['bathroom', 'organizing'], availability: 'mornings' } },
            { id: 'user5', name: 'David Kim', email: 'david.k@usc.edu', avatar: '👨‍💼', preferences: { chores: ['bills', 'maintenance'], availability: 'weekends' } },
            { id: 'user6', name: 'Lisa Wang', email: 'lisa.w@usc.edu', avatar: '👩‍🎨', preferences: { chores: ['decorating', 'plants'], availability: 'flexible' } },
            { id: 'user7', name: 'James Wilson', email: 'james.w@usc.edu', avatar: '👨‍🏫', preferences: { chores: ['trash', 'recycling'], availability: 'evenings' } },
            { id: 'user8', name: 'Maya Patel', email: 'maya.p@usc.edu', avatar: '👩‍⚕️', preferences: { chores: ['cleaning', 'dishes'], availability: 'mornings' } }
        ];
    ...
    renderInventory() {
        ...
        filteredItems.forEach(item => {
            ...
            const purchasedBy = this.roommates.find(r => r.id === item.purchasedBy)?.name || 'Unknown';
            ...
            itemElement.innerHTML = `
                ...
                <div class="inventory-item-details">
                    ...
                    <div class="inventory-detail">
                        ...
                        <div class="inventory-detail-value">${purchasedBy}</div>
                    </div>
                    <div class="inventory-detail">
    ...
    renderBills() {
        ...
        const billsToShow = this.filteredBills || this.bills;
        ...
        billsToShow.forEach(bill => {
            ...
            const paidBy = bill.paidBy === this.settings.currentUserId ? this.userProfile.name : (this.roommates.find(r => r.id === bill.paidBy)?.name || 'Unknown');
            ...
            billElement.innerHTML = `
                <div class="bill-item-header">
                    ...
                    <div class="bill-item-amount-container">
                        ...
                        <div class="bill-status ${isFullyPaid ? 'paid' : isOverdue ? 'overdue' : 'active'}">
                            ${isFullyPaid ? 'PAID' : isOverdue ? 'OVERDUE' : 'ACTIVE'}
                        </div>
                ...
                <div class="bill-item-details">
                    ...
                    <div class="bill-detail">
                        ...
                        <div class="bill-detail-value">${totalPaid}/${totalSplits} Paid</div>
                    </div>
                </div>
                    ...
                    ${bill.splits.map(split => {
                        ...
                        return `
                            <div class="bill-split">
                                <span class="bill-split-name">${roommate?.name || 'Unknown'}</span>
                                <span class="bill-split-amount ${split.paid ? 'paid' : 'unpaid'}">
                                    $${split.amount.toFixed(2)} ${split.paid ? '✓' : '⏳'}
                                </span>
                            </div>
                        `;
                    }).join('')}
                ...
                <div class="bill-item-actions">
                    ${bill.createdBy === this.userProfile.name ? `
                        <button class="btn btn-small btn-primary" onclick="app.editBill('${bill.id}')">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                    ` : ''}
                    <button class="btn btn-small btn-secondary" onclick="app.markBillAsPaid('${bill.id}')">
                        <i class="fas fa-check"></i> Mark Paid
                    </button>
                    <button class="btn btn-small btn-success" onclick="app.settleUpBill('${bill.id}')">
    ...
    searchBills(searchTerm) {
        if (!searchTerm.trim()) {
            ...
            this.filteredBills = this.bills.filter(bill => {
                const paidBy = bill.paidBy === this.settings.currentUserId ? this.userProfile.name : (this.roommates.find(r => r.id === bill.paidBy)?.name || 'Unknown');
                return bill.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    renderUpcomingEvents(container) {
        ...
        this.events.forEach(event => {
            ...
            const creator = this.roommates.find(r => r.id === event.creator)?.name || 'Unknown';
            ...
            eventElement.innerHTML = `
                ...
                <div class="event-item-details">
                    ...
                    <div class="event-item-creator">Created by ${creator}</div>
                </div>
            `;
    ...
    renderPolls(container) {
        ...
        this.polls.forEach(poll => {
            ...
            const creator = this.roommates.find(r => r.id === poll.creator)?.name || 'Unknown';
            ...
            pollElement.innerHTML = `
                ...
                <div class="poll-item-details">
                    ...
                    <div class="poll-item-creator">Created by ${creator}</div>
                </div>
    ...
    renderChat(container) {
        container.innerHTML = '<p class="text-center">Chat feature coming soon!</p>';
    }
        ...
        this.roommates.forEach(roommate => {
            ...
            roommateElement.innerHTML = `
                <div class="roommate-card-header">
                    <div class="roommate-avatar" onclick="app.openRoommateProfile('${roommate.id}')">
                        ${roommate.avatar && roommate.avatar.startsWith('data:') ? 
                            ...
                            `<div class="avatar-placeholder">${roommate.avatar || '👤'}</div>`
                        }
                    </div>
                    <div class="roommate-info">
                        <h3>${roommate.name}${roommate.id === this.settings.currentUserId ? ' (You)' : ''}</h3>
                        <p>${roommate.email}</p>
                    </div>
                    <div class="roommate-actions">
                        <button class="btn btn-small btn-secondary" onclick="app.openRoommateProfile('${roommate.id}')">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn btn-small btn-danger" onclick="app.removeRoommate('${roommate.id}')">
                ...
                <div class="roommate-stats">
                    <div class="stat-item">
                        <span class="stat-value">${stats.choresCompleted}</span>
                        <span class="stat-label">Chores Done</span>
                    </div>
                    <div class="stat-item">
                        <span class="stat-value">${stats.billsPaid}</span>
                        <span class="stat-label">Bills Paid</span>
                    </div>
                </div>
            `;
    ...
    openRoommateProfile(roommateId) {
        ...
        // Pre-fill the profile modal with roommate data
        document.getElementById('profile-name-input').value = roommate.name;
        document.getElementById('profile-email-input').value = roommate.email;
        
        // Set current user for editing
        this.settings.currentUserId = roommateId;
        this.updateProfileDisplay();
    ...
    markAsRead(notificationId) {
        ...
        if (notification) {
            ...
            // Refresh the entire panel to show updated counts and remove check button
            this.showNotificationsPanel();
    ...
    deleteNotification(notificationId) {
        if (confirm('Are you sure you want to delete this notification?')) {
            ...
            // Refresh the entire panel to show updated counts
            this.showNotificationsPanel();
